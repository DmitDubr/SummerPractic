...
- [x] [BE-03. Довести БД, миграции и sqlc](#be-03-довести-бд-миграции-и-sqlc)
- [x] [BE-04. Реализовать сессии ClientSession (JWT)](#be-04-реализовать-сессии-clientsession-jwt)
...
- [x] [BE-09. Реализовать отмену брони (правило 1 часа)](#be-09-реализовать-отмену-брони-правило-1-часа) — *LOGIC-004 уточнён в BE-17c*
- [x] [BE-10. Реализовать лист ожидания](#be-10-реализовать-лист-ожидания) — *NOTIFIED: BE-17d*
- [x] [BE-11. Реализовать оценки инструкторов](#be-11-реализовать-оценки-инструкторов)
- [x] [BE-12. Реализовать push-токены и dev-симуляции](#be-12-реализовать-push-токены-и-dev-симуляции)
- [x] [BE-13. Довести контрактные ошибки и валидацию](#be-13-довести-контрактные-ошибки-и-валидацию) — *SLOT_REBOOK_FORBIDDEN: BE-17a*
- [x] [BE-14. Добавить полный набор Go-тестов](#be-14-добавить-полный-набор-go-тестов) — *интеграционные: BE-17h, нужен Docker PG*
- [x] [BE-15. Подготовить локальный запуск и документацию](#be-15-подготовить-локальный-запуск-и-документацию)
- [x] [BE-16. Финальная проверка готовности BE](#be-16-финальная-проверка-готовности-be)
- [ ] [BE-17. Закрытие пробелов по аудиту ТЗ](#be-17-закрытие-пробелов-по-аудиту-тз)
  - [x] [BE-17a. `SLOT_REBOOK_FORBIDDEN` (FR-011, R-008)](#be-17a-slot_rebook_forbidden-fr-011-r-008)
  - [x] [BE-17b. Полный `getBooking`: `priceBreakdown`, `waitlistPosition`](#be-17b-полный-getbooking-pricebreakdown-waitlistposition)
  - [x] [BE-17c. LOGIC-004: ранняя отмена освобождает место, поздняя — нет](#be-17c-logic-004-ранняя-отмена-освобождает-место-поздняя--нет)
  - [x] [BE-17d. Waitlist: `NOTIFIED` при освобождении места (BE-10)](#be-17d-waitlist-notified-при-освобождении-места-be-10)
  - [x] [BE-17e. `ONE_BOOKING_PER_DAY` при `joinWaitlist` (Q 1.3)](#be-17e-one_booking_per_day-при-joinwaitlist-q-13)
  - [x] [BE-17f. Прокат: списание инвентаря при RENTAL-брони (Q 2.4)](#be-17f-прокат-списание-инвентаря-при-rental-брони-q-24)
  - [x] [BE-17g. `joinWaitlist` → booking `WAITLIST` + `leaveWaitlist` по статусу](#be-17g-joinwaitlist--booking-waitlist--leavewaitlist-по-статусу)
  - [ ] [BE-17h. Интеграционные тесты по новым правилам](#be-17h-интеграционные-тесты-по-новым-правилам)
...
| BE-17 | Аудит ТЗ (LOGIC-004, FR-011, Q 1.3/2.4, BE-08/10) | SCR-007, 008, 009, 012 |
...
4. **BE-10…BE-12** — waitlist, рейтинги, push.
5. **BE-17** — закрытие пробелов по аудиту соответствия ТЗ (см. ниже).

---

## BE-17. Закрытие пробелов по аудиту ТЗ

> Итерация добавлена после сверки с `01-analysis/` и smoke-тестами. Часть пунктов BE-03…BE-16 была отмечена выполненной, но не покрывала критерии LOGIC-004, FR-011 и полноту контракта `getBooking`.

### Статус аудита (исходный)

| Область | Было | Цель BE-17 |
|---------|------|------------|
| `SLOT_REBOOK_FORBIDDEN` | enum есть, не возвращается | 403 при повторной записи после `CANCELLED_BY_GYM` |
| `getBooking` | без `priceBreakdown`, `waitlistPosition` | полный `Booking` по `schemas.yaml` |
| LOGIC-004 отмена | триггер всегда освобождал место | ранняя (≥1 ч) — да; поздняя — нет |
| Waitlist notify | `NOTIFIED` не выставлялся | первый в очереди → `NOTIFIED` при ранней отмене |
| `joinWaitlist` | без лимита 1 запись/день | `ONE_BOOKING_PER_DAY` |
| Прокат RENTAL | проверка без списания | decrement `rental_availability` |
| `leaveWaitlist` | по slot из любой брони | только `status=WAITLIST` |
| WAITLIST в «Мои записи» | только `waitlist_entries` | booking `WAITLIST` при join |

---

### BE-17a. `SLOT_REBOOK_FORBIDDEN` (FR-011, R-008)

Сделать:

- В `CreateBooking` до pre-check слота: если у клиента есть бронь на этот `slot_id` со статусом `CANCELLED_BY_GYM` → `403 SLOT_REBOOK_FORBIDDEN`.
- Если `slot.status = CANCELLED` → по-прежнему `409 SLOT_CANCELLED`.

Готово, когда:

- Integration: gym-cancel → попытка create → 403 `SLOT_REBOOK_FORBIDDEN`.

---

### BE-17b. Полный `getBooking`: `priceBreakdown`, `waitlistPosition`

Сделать:

- `GetBooking`: вычислять `priceBreakdown` из тарифа слота + `equipment` брони (LOGIC-003).
- `waitlistPosition` — из `waitlist_entries` при `status=WAITLIST` или активной записи в очереди.

Готово, когда:

- `GET /bookings/{id}` возвращает оба поля по контракту.

---

### BE-17c. LOGIC-004: ранняя отмена освобождает место, поздняя — нет

Сделать:

- Миграция: `trg_bookings_release_slot` освобождает место только если:
  - `CANCELLED_BY_GYM`, или
  - `CANCELLED_BY_CLIENT` и до `starts_at` ≥ 1 час.
- Поздняя отмена: статус меняется, `free_spots` **не** увеличивается.

Готово, когда:

- Integration: ранняя отмена → `free_spots +1`; поздняя → `free_spots` без изменений.

---

### BE-17d. Waitlist: `NOTIFIED` при освобождении места (BE-10)

Сделать:

- После ранней отмены (место освобождено): первый `WAITING` → `NOTIFIED`.
- Лог push-заглушки (`slog`) с `client_id` и `slot_id` (до BE-12 production push).

Готово, когда:

- Integration: join waitlist → early cancel другой брони → entry `NOTIFIED`.

---

### BE-17e. `ONE_BOOKING_PER_DAY` при `joinWaitlist` (Q 1.3)

Сделать:

- `joinWaitlist`: проверка `checkOneBookingPerDay` (ACTIVE) и отсутствия другой `WAITLIST`/`ACTIVE` на тот же календарный день.

Готово, когда:

- Второй join/бронь в тот же день → `409 ONE_BOOKING_PER_DAY`.

---

### BE-17f. Прокат: списание инвентаря при RENTAL-брони (Q 2.4)

Сделать:

- В `trg_bookings_reserve_slot` (или TX create): при `RENTAL` уменьшать `shoes_available` / `harness_available`.

Готово, когда:

- Две RENTAL-брони с исчерпанием фонда → вторая `RENTAL_UNAVAILABLE`.

---

### BE-17g. `joinWaitlist` → booking `WAITLIST` + `leaveWaitlist` по статусу

Сделать:

- `joinWaitlist`: insert `bookings` со статусом `WAITLIST` (для SCR-008, `leaveWaitlist`).
- `leaveWaitlist`: только если `booking.status == WAITLIST`.
- `deleteWaitlistEntry`: синхронно отменять связанную `WAITLIST`-бронь.

Готово, когда:

- `GET /bookings?status=WAITLIST` возвращает записи после join.
- `leaveWaitlist` на ACTIVE → 409.

---

### BE-17h. Интеграционные тесты по новым правилам

Сделать:

- Тесты: rebook forbidden, early/late cancel spots, waitlist notify, join limit per day.
- Документировать `TEST_DATABASE_URL` для Docker Postgres (порт 5432 vs локальный PG).

Готово, когда:

- `go test ./internal/handler/... -run Integration` проходит с `TEST_DATABASE_URL`.
...
| Go-код `backend/` | BE-00…BE-16 + **BE-17a–g** (миграция `000003_tz_compliance`) |
