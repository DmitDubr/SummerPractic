...
| `isRegularClient = true` | Бейдж «Постоянный клиент» (только отображение, Q 7.3) |
...
| `profile.briefingCompleted` | boolean | in | Read-only; влияет на блок инструктажа (Q 1.7) |
...
| ID | Описание |
|----|----------|
| FR-006 | Запись с контактными данными |
| FR-019 | Метка постоянного клиента |
| FR-022 | Операции профиля через API |
| Q 1.1 | Имя + телефон достаточно для идентификации |
| Q 7.3 | Бейдж постоянного клиента без привилегий |
