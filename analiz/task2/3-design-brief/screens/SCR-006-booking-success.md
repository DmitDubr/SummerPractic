...
Экран подтверждает клиенту успешное создание брони на кулинарный класс после ответа `createBooking` (201). Показывает **сводку класса и цену** с явной подписью «К оплате на месте». После **первой успешной записи** на устройстве — блок запроса push-разрешения (FR-027). Закрывает успешную ветку FR-009 и задаёт следующий шаг: просмотр «Мои записи» или возврат к расписанию.

Полноэкранный success-state предпочтительнее modal — ясное завершение потока записи.

### User Story

> Как клиент, я хочу увидеть подтверждение записи и сводку класса, чтобы быть уверенным, что бронь создана, и перейти к следующему действию.
...
| OS | «Добавить в календарь» (Could) | Intent в календарь с адресом студии |
...
| `slot` | SlotDetail | Передано из SCR-005 / `createBooking` | Дата, программа, шеф, длительность для сводки |
...
| :-- | :-- | :-- |
| [LOGIC-003_Цена-класса](../../5-mobile-app-spec/09_Логики/LOGIC-003_Цена-класса.md) | Блок «К оплате на месте» | Сумма из `booking.totalPrice`; прокат не влияет на цену |
| [LOGIC-007_Запрос-push-разрешения](../../5-mobile-app-spec/09_Логики/LOGIC-007_Запрос-push-разрешения.md) | Блок «Напоминания о классе» | Показ только при первой записи; CTA → системный диалог → `registerPushToken` |
...
│    новичков                     │
│ 👨‍🍳 Шеф: Мария                 │
│ 🎒 Со своим / Прокат: фартук,   │
│    набор ножей                  │
│ 💰 К оплате на месте: 3 500 ₽   │
│ [ Записан ]                     │
├─────────────────────────────────┤
│ 🔔 Напоминания о классе         │  ← только первая запись
...
| Сводка брони | Дата, время, длительность, программа, шеф, экипировка | `slot.*`, `booking.equipment` | Дата/время — крупнее остального |
| Бейдж «Записан» | Статус брони | `booking.status` | Согласован со статусами SCR-008 |
| Строка экипировки | OWN или перечень проката | `booking.equipment.mode` | OWN → «Со своим (фартук и ножи)»; RENTAL → «Прокат: …» |
| Блок цены | Сумма к оплате на месте | `booking.totalPrice` | Формат «К оплате на месте: X ₽»; без разбивки проката |
| Блок push-разрешения | Объяснение типов уведомлений FR-027 | Локально + [LOGIC-007](../../5-mobile-app-spec/09_Логики/LOGIC-007_Запрос-push-разрешения.md) | Только при `isFirstBooking` и `pushPermissionRequested = false` |
...
| Подпись «Мы напомним о классе» | Для клиентов с уже выданным разрешением push | OS permission | Показывается вместо блока push |
| CTA «Мои записи» | Primary — переход к списку с новой записью | Навигация | Сброс стека; SCR-008 с новой бронью вверху |
| CTA «К расписанию» | Secondary — возврат к поиску классов | Навигация | Сброс стека до SCR-001 |
| «Добавить в календарь» | Intent в календарь OS с адресом студии | Локально + `slot` | Could; скрыть при недоступности intent |

**Терминология:** **шеф**, **программа класса**, **тип кухни**; прокат (фартук, ножи) **не влияет на цену** (FR-015).

**Критерии для дизайнера:**

- Тон экрана — позитивный, тёплый (кулинарная студия); избегать излишней «праздничной» анимации.
- Сводка — карточка с чёткой иерархией: **дата/время крупнее** остального; программа и шеф — вторичный уровень.
...
- После успеха бэкенд отправляет push «подтверждение записи» (FR-027) — можно упомянуть в подписи для клиентов с разрешением.
...
| :-- | :-- | :-- |
| Content (успех, своё) | `equipment.mode = OWN` | Сводка: «Со своим (фартук и ножи)» |
| Content (успех, прокат) | `equipment.mode = RENTAL` | «Прокат: фартук, набор ножей» (перечень выбранного) |
| Push primer | `isFirstBooking = true`, push не запрашивался | Блок «Напоминания о классе» + CTA |
| Push hidden (повторная запись) | Не первая бронь | Блок push **скрыт** |
| Push granted | OS permission granted | Блок push скрыт; подпись «Мы напомним о классе» |
...
5. **Календарь (Could):** тап «Добавить в календарь» → событие с названием программы, временем, адресом студии «Шеф-стол».
...
| FR-015 | Сводка содержит цену класса; явно «К оплате на месте»; прокат не влияет на сумму |
| FR-027 | Запрос push после первой записи; типы: напоминание за день, за 2 ч, подтверждение записи, отмена, перенос, запрос аллергий |
| NFR-010 | Только push, Android |
| UC-002 | Успешное завершение сценария записи |
| US-005 | Пользователь получает подтверждение брони |
...
| ID | Критерий |
| :-- | :-- |
| AC-001 | **Дано** `createBooking` вернул 201, **Когда** открывается SCR-006, **Тогда** отображаются иконка успеха, заголовок «Вы записаны!» и сводка с датой, программой, шефом и `booking.totalPrice`. |
...
| AC-003 | **Дано** `isFirstBooking = true` и push ещё не запрашивался, **Когда** SCR-006 в Content, **Тогда** показан блок «Напоминания о классе» с CTA «Включить уведомления» и «Не сейчас». |
...
| AC-008 | **Дано** push уже разрешён в OS, **Когда** SCR-006 открыт после первой записи, **Тогда** блок primer скрыт и показана подпись «Мы напомним о классе». |
