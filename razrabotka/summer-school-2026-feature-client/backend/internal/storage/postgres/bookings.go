...
SELECT EXISTS (
	...
	var priorBookings int
	if err := tx.QueryRow(ctx, `SELECT count(*) FROM bookings WHERE client_id = $1`, clientID).Scan(&priorBookings); err != nil {
		return booking.Booking{}, fmt.Errorf("count client bookings: %w", err)
	}
...
RETURNING id::text, created_at`, command.SlotID, clientID, command.SeatsCount, command.RentalCount, now).Scan(&bookingID, &createdAt)
	...
	created.IsFirstBooking = isFirstBooking
	created.ReminderHours = []int{24, 2}
