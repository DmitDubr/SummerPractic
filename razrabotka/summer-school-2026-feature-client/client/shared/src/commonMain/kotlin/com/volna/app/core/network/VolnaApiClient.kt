...
internal fun Throwable.toAppFailureException(): AppFailureException = when (this) {
    ...
    is SerializationException -> AppFailureException(AppFailure.Unknown)
        ...
        message.isBrowserFetchFailure() -> AppFailureException(AppFailure.NetworkUnavailable)
        else -> AppFailureException(AppFailure.Unknown)
    }
}

internal fun String?.isBrowserFetchFailure(): Boolean {
    if (this == null) return false
    val normalized = lowercase()
    return normalized.contains("fail to fetch") ||
        normalized.contains("networkerror") ||
        normalized.contains("load failed") ||
